...
partial class RulesetEditor : Form
{
    ...
    public RulesetEditor(WordService wordService, RuleSet ruleset)
    {
        InitializeComponent();
        ...
        AllowFlip.Checked = ruleset.AllowFlip;
        ValidateWords.Checked = ruleset.ValidateWords;
        Player1Name.Text = ruleset.Player1Name;
        Player2Name.Text = ruleset.Player2Name;

        SetUpBoardEditor();

        Distribution.Clear();

        foreach (var item in ruleset.LetterDistribution.OrderBy(x => x.Key))
        {
            Distribution.Add((item.Key, ruleset.LetterDisplay[item.Key] ?? " ", item.Value, RuleSet.LetterPoints.GetValueOrDefault(item.Key, 0)));
        }
    ...
    private void ContinueButton_Click(object sender, EventArgs e)
    {
        ...
        RuleSet.AllowFlip = AllowFlip.Checked;
        RuleSet.ValidateWords = ValidateWords.Checked;
        RuleSet.Player1Name = Player1Name.Text;
        RuleSet.Player2Name = Player2Name.Text;

        RuleSet.LetterDistribution.Clear();
        RuleSet.LetterPoints.Clear();
        RuleSet.LetterDisplay.Clear();

        foreach (var (letter, display, count, points) in Distribution)
        {
            if (letter.Trim() != "")
            {
                RuleSet.LetterDistribution[letter] = count;
                RuleSet.LetterDisplay[letter] = display == "" ? " " : display;
                RuleSet.LetterPoints[letter] = points;
            }
        }
    ...
    {
        RuleSet.BoardSize = (int)BoardSize.Value;
    ...
    private void SetUpBoardEditor()
    {
        ...
        for (int i = 0; i < BoardEditor.ColumnCount; i++)
        {
            ...
            {
                void SetColour(Button button, int type)
                {
    ...
    private void AllowFlip_CheckedChanged(object sender, EventArgs e)
    {
        RuleSet.AllowFlip = AllowFlip.Checked;
    }
    ...
    {
        RuleSet.BoardSize = WordService.RuleSets[0].BoardSize;
        RuleSet.Board = ResizeArray(RuleSet.Board, (int)BoardSize.Value, (int)BoardSize.Value);
        ...
        for (int i = 0; i < RuleSet.BoardSize; i++)
        {
            for (int j = 0; j < RuleSet.BoardSize; j++)
            {
                RuleSet.Board[j, i] = WordService.RuleSets[0].Board[j, i];
            }
        }
    ...
    {
        RuleSet.LetterDisplay = new(WordService.RuleSets[0].LetterDisplay);
        RuleSet.LetterDistribution = new(WordService.RuleSets[0].LetterDistribution);
        RuleSet.LetterPoints = new(WordService.RuleSets[0].LetterPoints);
    ...
    {
        string filename = @"C:\Users\huggl\wirds\rules\variant.txt";
        ...
        foreach (string line in File.ReadLines(filename))
        {
            ...
            {
                AllowFlip.Checked = bool.Parse(parms[0]);
            }
    ...
    private void SaveButton_Click(object sender, EventArgs e)
    {
        ...
        writer.WriteLine("flip:" + AllowFlip.Checked);
